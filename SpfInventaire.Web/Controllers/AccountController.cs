...
namespace SpfInventaire.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                        Session.Add(Constantes.SESSION_ID_USER, User.Identity.GetUserId());
                    return RedirectToAction("Index", "Home");
                case SignInStatus.LockedOut:
                    Session.Remove(Constantes.SESSION_ID_USER);
                    return View("Lockout");
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { Nom = model.Nom, Prenom = model.Prenom, Surnom = model.Surnom, UserName = model.UserName, Email = model.Email };
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                await UserManager.SendEmailAsync(user.Id, "réinitialisation mot de passe", "Merci de cliquer sur le lien suivant pour réinitialiser votre mot de passe:" + callbackUrl);
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            var user = await UserManager.FindByNameAsync(model.Matricule);
            if (user == null)
            {
